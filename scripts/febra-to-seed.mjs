...
import { parsePromptBlocks, febraToEntities } from '../src/lib/febra.js';
...
const FAMILIES = [
  ...
  {
    family: 'Line',
    rows: readJson('bundles_Line_FEBRA_En.json'),
    serviceClasses: ['red'],
    translations: translations({ ar: 'Line FEBRA Ar', ckb: 'Line FEBRA Ku' }),
  },
];

const { entities, skipped, notes } = febraToEntities(FAMILIES);
...
say();
say(`**${entities.length} of ${FAMILIES.reduce((n, f) => n + f.rows.length, 0)} rows imported**`
  + ` — ATL ${byFamily('ATL')}, Yooz ${byFamily('Yooz')}, Line ${byFamily('Line')}.`);
say();
...
say();
say('- **`how_to.subscribe` / `how_to.unsubscribe`** — the ATL and Yooz exports carry no');
say('  subscription steps at all. The Line rows DO ("send 1 to 230, or dial `*230#`") but');
say('  those rows have no bundleId and could not be imported; harvest them by hand.');
...
// ── write ────────────────────────────────────────────────────────────────────
if (DRY) {
  console.log(`\n--dry-run: would write ${entities.length} files to ${path.relative(ROOT_DIR, OUT_DIR)}`);
} else {
  ...
  console.log(`\nWrote ${entities.length} entities to ${path.relative(ROOT_DIR, OUT_DIR)}`);

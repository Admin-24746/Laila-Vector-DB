// Retrieval service — the stateless REST contract Druid calls (docs/08), plus the
// flow-builder sandbox console (docs/20 §4) at GET /.
...
async function audit(record, file = 'service.jsonl') {
...
app.addHook('onRequest', async (req, reply) => {
  if (!CONFIG.serviceToken || !req.url.startsWith('/v1/')) return;
...
app.post('/v1/retrieve', async (req) => {
  ...
  const u = await understandQuery(text, history, language);
  const results = await mergedRetrieve(u, {
  ...
  if (expand && results[0]) {
    response.entity_card = await entityCard(results[0].entity_id, results[0].language);
  }
  await audit({
...
app.post('/v1/route', async (req) => {
  ...
  const u = await understandQuery(text, history, language);
  const { matches, ...decision } = await routeMessage(u.rewritten ?? u.raw);
  const response = {
    ...decision,
    query_understanding: understandingMeta(u),
    latency_ms: Date.now() - t0,
...
app.post('/v1/answer', async (req, reply) => {
  ...
  const u = await understandQuery(text, history, language);
  const results = await mergedRetrieve(u, { topK: top_k ?? CONFIG.topK, filters });
...
app.get('/healthz', async (req, reply) => {
  const [qdrant, tei] = await Promise.all([qdrantHealthy(), embedderHealthy()]);
  const ok = qdrant && tei;
  if (!ok) reply.code(503);
  let points = null;
  if (qdrant) points = await countPoints().catch(() => null);
  return { ok, qdrant, tei, llm: llmConfigured(), collection: CONFIG.collection, points };
});
